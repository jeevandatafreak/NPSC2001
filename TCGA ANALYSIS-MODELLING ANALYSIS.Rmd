...
fviz_eig(cancer_data1_pca, 
         ...
         barcolor="black", 
         linecolor="purple", 
         ncp=15, labelsize = 13, repel = TRUE)

#Effecitvely after the seventh principal component, the percentage slightly decreases, all the way to the 81st.
```

```{r}
cancer_data1_pca %>% fviz_pca_ind(col.ind = "cos2") + scale_color_gradient2(low="cyan", mid="blue",
      high="black", midpoint=0.6)+ theme_minimal()
```

```{r}
fviz_pca_ind(cancer_data1_pca, geom.ind = "point", pointshape = 21, 
             pointsize = 1.5, 
             fill.ind = cancer_data2$Subtype, 
             col.ind = cancer_data2$Subtype, 
             palette = "BrBG", 
             addEllipses = TRUE,
             label = "var",
             col.var = "blue",
  ...
  ggtitle("2D PCA-plot from 80 components") 
```
...
```{r}
#Out of curiousity, let's plot the final two principal component analysis. Essentially, these are the least 'reflective' of the dimension of the data, hence it would be interesting to see if they're similar to the more representative principal components above.

ggplot(cancer_data_pc, aes(PC79, PC80, col = Subtype)) +
  geom_point(aes(shape = Subtype), size = 4) +
  coord_cartesian(xlim = 1.5 * c(min(cancer_data_pc$PC79), max(cancer_data_pc$PC79)),
                  ylim = 1.5 * c(min(cancer_data_pc$PC80), max(cancer_data_pc$PC80))) +
    geom_encircle(data = cancer_Basal, aes(x=PC79, y=PC80)) +   # draw circles
   geom_encircle(data = cancer_LumA, aes(x=PC79, y=PC80)) + 
   geom_encircle(data = cancer_LumB, aes(x=PC79, y=PC80)) +
     geom_encircle(data = cancer_HER2, aes(x=PC79, y=PC80)) +
  theme_bw() + 
  labs(title = "Principal Component Analysis of Cancer Subtypes (PC79 vs PC80)",
              x = paste0("PC79 - ", pca_variance_pcent[79], " % (variation accounted)"),
       y = paste0("PC80 - ", pca_variance_pcent[80], " % (variation accounted)"))


#As shown below, the final to principal components display very largely spread values. Luminal B and luminal A appear to be quite similar while Basal-like and HER2 share some similar shape. If you look at the y axis scale, the PCA goes to values of nearly 0 (2 x 10 ^-13), essentially displaying absolute minimum variance. Essentially PCA is used to maximise variance to ensure that most data can be accounted for, but in this case it is the least and provides the least statistical information. Variance IS required to make statistical insights as it is the component that provides any comparison within the data.
...
```{r}
#Make factor levels for subtype
cancer_data3 %>% 
...
```{r}
#This is to speed up the process of analysis. UNNECESSARY TO RUN FOR OTHER PCs
...
#Can't plot as only one accuracy given. 65% accurate, far less than random forest but closer to log Boost 
```
...
```{r}
predict_cancer_nnet <- predict(caret_model_nnet, newdata = cancer_test) #As you can see, we're using the logBoost model generation through the caret package. Will only be done for logBoost since it has no external package.

table_cancer_nnet <- table(Actual = cancer_test_truth, Predicted = predict_cancer_nnet)

cmCancer_nnet <- confusionMatrix(table_cancer_nnet, reference = "Subtype")

pltCancer_nnet <- as.data.frame(cmCancer_nnet$table)

pltCancer_nnet$Predicted <- factor(pltCancer_nnet$Predicted, levels=rev(levels(pltCancer_nnet$Predicted)))
...
ggplot(pltCancer_nnet, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "Neural Network prediction of Subtypes")

#Interpretation: As shown below, considering an initial accuracy display of 51%, it appears that neural net produces a near 100% accuracy for the subtype prediction. This demonstrates how important the predict function is, in which the test data references against generated predicted values, which may determine different accuracy than the one given within the caret model output. 96.6% Accuracy.
```

```{r}
PRROC_obj_nnet <- roc.curve(scores.class0 = pltCancer_nnet$Predicted, weights.class0=pltCancer_nnet$Freq, curve = T)
plot(PRROC_obj_nnet, main = "ROC curve - Neural Networks")
```



```{r}
predict_cancer_nb <- predict(caret_model_nb, newdata = cancer_test) #As you can see, we're using the logBoost model generation through the caret package. Will only be done for logBoost since it has no external package.

table_cancer_nb <- table(Actual = cancer_test_truth, Predicted = predict_cancer_nb)

cmCancer_nb <- confusionMatrix(table_cancer_nb, reference = "Subtype")

pltCancer_nb <- as.data.frame(cmCancer_nb$table)

pltCancer_nb$Predicted <- factor(pltCancer_nb$Predicted, levels=rev(levels(pltCancer_nb$Predicted)))
...
ggplot(pltCancer_nb, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "Naive Bayes prediction of Subtypes")

#Interpretation: The naive bayes model appears to have a 73.9% accuracy. It's errors in predictions appear to be 'one-off', essentially where there isn't a consistent error in prediction for one subtype over the other. This can be seen as all values with a number other than the diagonal only appear to be 1.

```

```{r}
PRROC_obj_nb <- roc.curve(scores.class0 = pltCancer_nb$Predicted, weights.class0=pltCancer_nb$Freq, curve = T)
plot(PRROC_obj_nb, main = "ROC curve - Naive Bayes")
```
...
PRROC_obj_rf <- roc.curve(scores.class0 = pltCancer_rf$Predicted, weights.class0=pltCancer_rf$Freq, curve = T)
plot(PRROC_obj_rf, main = "ROC curve - Random Forests")
...
PRROC_obj_logB <- roc.curve(scores.class0 = pltCancer_logB$Predicted, weights.class0=pltCancer_logB$Freq, curve = T)
plot(PRROC_obj_logB,main = "ROC curve - Log Boosted")
```
...
```{r}
predict_cancer_rpart <- predict(caret_model_rpart, newdata = cancer_test) #As you can see, we're using the logBoost model generation through the caret package. Will only be done for logBoost since it has no external package.

table_cancer_rpart <- table(Actual = cancer_test_truth, Predicted = predict_cancer_rpart)

cmCancer_rpart <- confusionMatrix(table_cancer_rpart, reference = "Subtype")

pltCancer_rpart <- as.data.frame(cmCancer_rpart$table)

pltCancer_rpart$Predicted <- factor(pltCancer_rpart$Predicted, levels=rev(levels(pltCancer_rpart$Predicted)))
...
ggplot(pltCancer_rpart, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "Rpart prediction of Cancer Types")

#Interpretation: 52% accuracy


```

```{r}
PRROC_obj_rpart <- roc.curve(scores.class0 = pltCancer_rpart$Predicted, weights.class0=pltCancer_rpart$Freq, curve = T)
plot(PRROC_obj_rpart, main = "ROC curve - Rpart")
```

#CROSS VALIDATION

_What needs to be done now is cross validation on the neural net and logboost prediction models. Though extremely good results, it is uncommon to have 100% accuracy using prediction models, hence cross validation occurs to check the validity of the model a 2nd time. This will be done by implementing a 5-fold cross-validation control, and a train/test split of 80%/20%._
...
#64/16 split required
```

```{r}
cancer_dataCross <- data.frame(cancer_data1, Subtype = cancer_data$PAM50.mRNA)

cancer_dataCross <- cancer_dataCross[,-1] #remove TCGA-ID


scrambled_indexCross <- sample(NROW(cancer_dataCross)) #mix up the rows 
scrambled_cancerCross <- cancer_dataCross[scrambled_indexCross,] #using that index, rows should be mixed


cancer_trainCross <- scrambled_cancerCross[1:64, ] #take 52 variables for training set (65%)
cancer_testCross <- scrambled_cancerCross[65:80, -8671] #Take the final 35%, without the subtype column
cancer_test_truthCross <- scrambled_cancerCross[65:80, 8671] #take the subtypes
```



```{r}
myctrl1 <- trainControl(method = "cv",     
                       number = 5,
                       allowParallel = TRUE)
...
caret_model_logBoostCross <- train(Subtype ~ .,
                     data = cancer_dataCross,
                     method = "LogitBoost",
                     metric = "Accuracy",
                     trControl = myctrl1)
...
# caret_model_nnetCross <- train(Subtype ~ .,
#                      data = cancer_dataCross,
#                      method = "nnet",
#                      metric = "Accuracy",
#                      trControl = myctrl1)
# 
#There appears to be a running issue, hence we will only cross validate the log boost prediction

```

```{r}
predict_cancer_logBCross <- predict(caret_model_logBoostCross, newdata = cancer_testCross) #As you can see, we're using the logBoost model generation through the caret package. Will only be done for logBoost since it has no external package.
```

```{r}
table_cancer_logBCross <- table(Actual = cancer_test_truthCross, Predicted = predict_cancer_logBCross)

cmCancer_logBCross <- confusionMatrix(table_cancer_logBCross, reference = "Subtype")

pltCancer_logBCross <- as.data.frame(cmCancer_logBCross$table)

pltCancer_logBCross$Predicted <- factor(pltCancer_logBCross$Predicted, levels=rev(levels(pltCancer_logBCross$Predicted)))
...
ggplot(pltCancer_logBCross, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "logBoost prediction of Cancer Types (80/20 Split)")

#Interpretation: Cross-validation confirms that the 100% Accuracy remains. Effectively, we can confirm that log Boost is extremely accurate in predicting cancer subtype.

```


```{r}
#We can use the $overall variable in each confusion matrix to output the given accuracies to 3 significant figures.

#FOR THE CANCER DATA
100*round(cmCancer_nnet$overall[1],3)
100*round(cmCancer_logB$overall[1],3)
100*round(cmCancer_rpart$overall[1],3)
100*round(cmCancer_nb$overall[1],3)
100*round(cmCancer_rf$overall[1],3)

```

```{r}
accuracies <- resamples(list(logB=caret_model_logBoost, Rpart =caret_model_rpart, NB=caret_model_nb, RF = caret_model_randomForest, NNet = caret_model_nnet))
...
_Below is further analysis not required in the final report._



#HORMONE RECEPTOR PREDICTION


```{r}
#After having completed prediction on the above data regarding Subtypes with the PAM50 genome, we can cross-reference to prediction capabilities of current biomarkers: the hormone receptors. Essentially, by applying the same method, we can compare the predictive accuracy of the above models and determine if they are successful in predicting hormone receptors as good as subtypes.

cancer_dataPR <- data.frame(cancer_data1, PRStatus = cancer_data$PR.Status)

cancer_dataPR <- cancer_dataPR[,-1] #remove TCGA-ID


scrambled_index1 <- sample(NROW(cancer_dataPR)) #mix up the rows 
scrambled_cancer1<- cancer_dataER[scrambled_index1,] #using that index, rows should be mixed


cancer_train1 <- scrambled_cancer1[1:52, ] #take 52 variables for training set (65%)
cancer_test1 <- scrambled_cancer1[53:80, -8671] #Take the final 35%, without the subtype column
cancer_test_truth1 <- scrambled_cancer1[53:80, 8671] #take the subtypes

# cancer_dataER %>% 
#   mutate(ERStatus = factor(ERStatus, 
#                         labels = make.names(levels(ERStatus))))
```


```{r}
caret_model_logBoostPR <- train(PRStatus ~ .,
                     data = cancer_dataPR,
                     method = "LogitBoost",
                     metric = "Accuracy",
                     trControl = myctrl)
...
```{r}
predict_cancer_logBPR <- predict(caret_model_logBoostPR, newdata = cancer_test1) #As you can see, we're using the logBoost model generation through the caret package. Will only be done for logBoost since it has no external package.

table_cancer_logBPR <- table(Actual = cancer_test_truth1, Predicted = predict_cancer_logBPR)

cmCancer_logBPR <- confusionMatrix(table_cancer_logBPR, reference = "PRStatus")


pltCancer_logBPR <- as.data.frame(cmCancer_logBPR$table)

pltCancer_logBPR$Predicted <- factor(pltCancer_logBPR$Predicted, levels=rev(levels(pltCancer_logBPR$Predicted)))
...
ggplot(pltCancer_logBPR, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "logBoost prediction of PR Hormone Status")

#Interpretation: Log Boost, again, produces 100% accuracy in determining progesterone receptors. This model effectively works for both subtypes and receptors equally.


```

```{r}
PRROC_obj_logBPR <- roc.curve(scores.class0 = pltCancer_logBPR$Predicted, weights.class0=pltCancer_logBPR$Freq, curve = T)
plot(PRROC_obj_logBPR, main = "ROC curve - Log Boost (Progesterone)")
```


```{r}
caret_model_nbPR <- train(PRStatus ~ .,
                     data = cancer_dataPR,
                     method = "nb",
                     metric = "Accuracy",
                     trControl = myctrl)
...
```{r}
predict_cancer_nbPR <- predict(caret_model_nbPR, newdata = cancer_test1) 

table_cancer_nbPR <- table(Actual = cancer_test_truth1, Predicted = predict_cancer_nbPR)

cmCancer_nbPR <- confusionMatrix(table_cancer_nbPR, reference = "PRStatus")

pltCancer_nbPR <- as.data.frame(cmCancer_nbPR$table)

pltCancer_nbPR$Predicted <- factor(pltCancer_nbPR$Predicted, levels=rev(levels(pltCancer_nbPR$Predicted)))
...
ggplot(pltCancer_nbPR, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "Naive Bayes prediction of PR Hormone Status")

#Interpretation: An accuracy of 96.6% for naive bayes. Again this is quite good. It's important to consider that it would be far easier to predict correct values considering there are only two possible values to predict.


```

```{r}
PRROC_obj_nbPR <- roc.curve(scores.class0 = pltCancer_nbPR$Predicted, weights.class0=pltCancer_nbPR$Freq, curve = T)
plot(PRROC_obj_nbPR, main = "ROC curve - Naive Bayes (Progesterone)")
```

# Tumor Stage Prediction: The same way we predicted hormone status, we may do with cancer stages. logBoost will be applied considering it had perfect accuracy in previous instances.

```{r}

cancer_dataStage <- data.frame(cancer_data1, Stage = cancer_data$AJCC.Stage)

cancer_dataStage <- cancer_dataStage[,-1] #remove TCGA-ID


scrambled_indexStage <- sample(NROW(cancer_dataStage)) #mix up the rows 
scrambled_cancerStage<- cancer_dataStage[scrambled_indexStage,] #using that index, rows should be mixed


cancer_trainStage <- scrambled_cancerStage[1:40, ] #We can do a 50/50 train/test split for the final prediction
cancer_testStage <- scrambled_cancerStage[41:80, -8671] #Take the final 35%, without the subtype column
cancer_test_truthStage <- scrambled_cancerStage[41:80, 8671] #take the subtypes

 cancer_dataStage %>% 
   mutate(Stage = factor(Stage, 
                         labels = make.names(levels(Stage))))
```


```{r}
caret_model_logBoostStage <- train(Stage ~ .,
                     data = cancer_dataStage,
                     method = "LogitBoost",
                     metric = "Accuracy",
                     trControl = myctrl)
```
...
```{r}
predict_cancer_logBStage <- predict(caret_model_logBoostStage, newdata = cancer_testStage) #As you can see, we're using the logBoost model generation through the caret package. Will only be done for logBoost since it has no external package.

table_cancer_logBStage <- table(Actual = cancer_test_truthStage, Predicted = predict_cancer_logBStage)

cmCancer_logBStage <- confusionMatrix(table_cancer_logBStage, reference = "PRStatus")


pltCancer_logBStage <- as.data.frame(cmCancer_logBStage$table)

pltCancer_logBStage$Predicted <- factor(pltCancer_logBStage$Predicted, levels=rev(levels(pltCancer_logBStage$Predicted)))
...
ggplot(pltCancer_logBStage, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "logBoost prediction of Cancer Stage") +
   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))

#Interpretation: 100% Accuracy given for cancer stages as well. It also confirms most values lie in Stage 2.


```

```{r}
PRROC_obj_logBStage <- roc.curve(scores.class0 = pltCancer_logBStage$Predicted, weights.class0=pltCancer_logBStage$Freq, curve = T)
plot(PRROC_obj_logBStage, main = "ROC curve - Log Boost (Stage)")
```
...
#UTILISING PCA FOR CLASSIFICATION PREDICTION - LINEAR DISCRIMINANT ANALYSIS/Random Forests

_Considering previously that the dataset is far too large for lda to occur, we can now use the principal components to apply lda prediction and determine its accuracy through the ROC curve. We will utilise both lda and random forest native functions._


```{r}
cancer_data1_pca_pred <- cancer_data1_pca$x[,1:80]
cancer_data1_pca_pred <- cbind(cancer_data1_pca_pred, as.numeric(cancer_data2$Subtype)-1)
colnames(cancer_data1_pca_pred)[81] <- "Subtype"

cancer_pca_pred_df <- as.data.frame(cancer_data1_pca_pred)
...
scrambled_index_pca <- sample(NROW(cancer_pca_pred_df)) #mix up the rows 
scrambled_cancer_pca<- cancer_pca_pred_df[scrambled_index_pca,] #using that index, rows should be mixed

cancer_train_pca <- scrambled_cancer_pca[1:52, ] #take 52 variables for training set (65%)
cancer_test_pca <- scrambled_cancer_pca[53:80, -81] #Take the final 35%, without the subtype column
cancer_test_truth_pca <- scrambled_cancer_pca[53:80, 81] #take the subtypes

cancer_test_pca[,ncol(cancer_test_pca)] #Checks that our final column doesn't contain subtypes

cancer_test_truth_pca #subtypes have been subsetted well

```

```{r}
cancer_pca_lda <- lda(Subtype ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6, data = cancer_train_pca)
cancer_pca_lda_predict <- predict(cancer_pca_lda, newdata = cancer_test_pca)
```


```{r}
table_cancer_pca_lda <- table(Actual = cancer_test_truth_pca, Predicted = cancer_pca_lda_predict$class)

cmCancer_pca_lda <- confusionMatrix(table_cancer_pca_lda, reference = "Subtype")

pltCancer_pca_lda <- as.data.frame(cmCancer_pca_lda$table)

pltCancer_pca_lda$Predicted <- factor(pltCancer_pca_lda$Predicted, levels=rev(levels(pltCancer_pca_lda$Predicted)))
```

```{r}
ggplot(pltCancer_pca_lda, aes(Predicted, Actual, fill= Freq)) + #Make confusion matrix, each value represents whether value was accurately predicted within the model
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction", title = "LDA prediction of cancer types (PCA)")
```

```{r}
PRROC_obj_lda_pca <- roc.curve(scores.class0 = pltCancer_pca_lda$Predicted, weights.class0=pltCancer_pca_lda$Freq, curve = T)
plot(PRROC_obj_lda_pca, main = "ROC curve - Linear Discriminant Analysis (on PCA)")

#It's not unexpected that the values are not extremely accurate, essentially we are using 80 variables rather than 8000. As such dimensionality reduction allowed for a higher AUC value, though prediction is not as good.

```

```{r}
#Finally, we can apply PCA to the progesterone receptor to see if it is distinguished enough from each other.

cancer_data1_pca_PR <- prcomp(cancer_data1[, 2:8671]) #include only numerics, don't need TCGA ID
...
cancer_data_pc_PR <- data.frame(cancer_data1_pca_PR$x, PR.Status = cancer_data$PR.Status)

cancer_Positive <- cancer_data_pc_PR[cancer_data_pc_PR$PR.Status == "Positive", ] #subsetting each type of cancer
cancer_Negative <- cancer_data_pc_PR[cancer_data_pc_PR$PR.Status == "Negative", ]
...
```{r}
ggplot(cancer_data_pc_PR, aes(PC1, PC2, col = PR.Status)) +
  geom_point(aes(shape = PR.Status), size = 4) +
  coord_cartesian(xlim = 1.5 * c(min(cancer_data_pc_PR$PC1), max(cancer_data_pc_PR$PC1)),
                  ylim = 1.5 * c(min(cancer_data_pc_PR$PC2), max(cancer_data_pc_PR$PC2))) +
    geom_encircle(data = cancer_Positive, aes(x=PC1, y=PC2)) +   # draw circles
   geom_encircle(data = cancer_Negative, aes(x=PC1, y=PC2) ) + 
  theme_minimal() + 
  labs(title = "Principal Component Analysis of Progesterone Status") #shortened for exporting (for poster)
